...
function filterByMonth<T extends { [K in DateKey]: Date }, DateKey extends keyof T>(
  items: T[],
  dateKey: DateKey,
  monthOffset: number
): T[] {
  ...
  return items.filter(item => {
    ...
    return itemDate.getMonth() === adjustedMonth && itemDate.getFullYear() === targetYear;
  });
}
...
export const getAnalytics = (
  ...
  const bothZero = invoicesOfThisMonth.length === 0 && invoicesOfLastMonth.length === 0;
  ...
  switch (analyticType) {
    case "invoicesValue":
      const sumInvoices = (invoices: Invoice[]) => invoices.reduce(
        (total, invoice) => total + sumAllProducts(invoice.products), 0
      );
      const lastMonthSum = sumInvoices(invoicesOfLastMonth);
      const thisMonthSum = sumInvoices(invoicesOfThisMonth);

      return {
        value: `${(thisMonthSum / invoicesOfThisMonth.length || 0).toFixed(2)} PLN`,
    ...
    case "clients":
      ...
      return {
        ...
        percent: bothZero ? 0 : calculatePercent(clientsOfThisMonth.length, clientsOfLastMonth.length),
    ...
    case "invoices":
      return {
        ...
        percent: bothZero ? 0 : calculatePercent(invoicesOfThisMonth.length, invoicesOfLastMonth.length),
